...
public class ApprovePaymentHandler {
    ...
    public ApprovePaymentHandler(PaymentRepository repo, ApplicationEventPublisher events) {
        this.repo = repo;
        this.events = events;
    }

    @Transactional
    public Payment handle(UUID id, ApprovePaymentCommand cmd) {
        Payment payment = repo.findById(id)
                .orElseThrow(() -> new NotFoundException("Payment not found: " + id));

        UUID cashierId = currentUserId();
        payment.approve(cmd.paymentMethod(), cashierId);
