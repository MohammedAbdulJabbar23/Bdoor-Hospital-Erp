...
/**
 * HMS-BRD-REC-005 — Premature admission spine driven END-TO-END THROUGH THE UI. Reception-seeded
 * incoming infant → premature staff admits via the dialog → cashier approves via the queue → bed
 * Occupied → click the bed (the new tabbed CASE PAGE) → extend + finish via the quick-action bar →
 * cashier approves the discharge → bed freed.
 */
...
test('full premature lifecycle through the UI: admit → pay → extend → finish → discharge → bed freed', async ({ page }) => {
  ...
  // --- Bed Occupied; open the case page, extend the stay, finish treatment via the quick actions. ---
  ...
  await page.getByTestId('detail-extend-value').fill('1');
  await page.getByTestId('detail-extend-unit').selectOption('DAYS');
  await page.getByTestId('detail-extend').click();
